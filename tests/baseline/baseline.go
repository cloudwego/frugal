...
func (p *DefaultValues) InitDefault() {
	p.ByteFieldWithDefault = 1
	p.I64FieldWithDefault = 2
	p.DoubleFieldWithDefault = 3.0
	p.I32FieldWithDefault = 4
	p.StringFieldWithDefault = "string field default text"
	p.BinaryFieldWithDefault = []byte("binary field default data")
	p.EnumFieldWithDefault = Enums_ValueA
	p.SimpleStructWithDefault = &Simple{
		ByteField:   10,
		I64Field:    11,
		DoubleField: 12.0,
		I32Field:    13,
	...
	p.MapI32I64WithDefault = map[int32]int64{
		1: 2,
		3: 4,
	}
	p.MapI64StringWithDefault = map[int64]string{
		1: "aaa",
		2: "bbb",
	}
	p.MapStringStringWithDefault = map[string]string{
		"aaa": "xxx",
		"bbb": "yyy",
	}
	p.MapStringSimpleWithDefault = map[string]*Simple{
		"aaa": &Simple{
			ByteField:   20,
			I64Field:    21,
			DoubleField: 22.0,
...
func (p *DefaultValues) writeField10(oprot thrift.TProtocol) (err error) {
	...
	for i := 0; i < len(p.SetFieldWithDefault); i++ {
		for j := i + 1; j < len(p.SetFieldWithDefault); j++ {
			if reflect.DeepEqual(p.SetFieldWithDefault[i], p.SetFieldWithDefault[j]) {
