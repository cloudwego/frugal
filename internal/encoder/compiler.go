...
func mkins(op OpCode, et defs.Tag, kt defs.Tag, id uint16, nb uint32, to int, vt *rt.GoType) Instr {
    return Instr(
        (uint64(id) << 48) |
        (uint64(nb) << 32) |
        (uint64(et) << 24) |
...
func mktype(v *rt.GoType) uint64 {
    if p := uintptr(unsafe.Pointer(v)); p > defs.MaxUint56 {
...
func (self Instr) Nb() uint32     { return uint32(self >> 32) }
...
func (self Instr) Disassemble() string {
    switch self.Op() {
        ...
        case OP_index          : return fmt.Sprintf("%-18s%d", self.Op(), self.Nb())
...
func (self *Program) pin(i int) { (*self)[i] = ((*self)[i] & 0xff) | Instr(uint64(self.pc()) << 32) }

func (self *Program) add(op OpCode)                            { self.ins(mkins(op, 0, 0, 0, 0, 0, nil)) }
func (self *Program) jmp(op OpCode, to int)                    { self.ins(mkins(op, 0, 0, 0, 0, to, nil)) }
func (self *Program) idx(op OpCode, nb uint32)                 { self.ins(mkins(op, 0, 0, 0, nb, 0, nil)) }
func (self *Program) seq(op OpCode, et defs.Tag)               { self.ins(mkins(op, et, 0, 0, 0, 0, nil)) }
func (self *Program) rtt(op OpCode, vt *rt.GoType)             { self.ins(mkins(op, 0, 0, 0, 0, 0, vt))   }
...
func (self *Compiler) compileOne(p *Program, sp int, vt *defs.Type) {
    switch vt.T {
        ...
        case defs.T_pointer : self.compilePtr     (p, sp, vt.V)
