...
func dumpbb(bb *BasicBlock, refs map[*Instr]string) string {
    ...
    for i := 0; i < bb.Len; i++ {
        buf = append(buf, fmt.Sprintf(`%p   %s\l`, p, p.disassemble(refs)))
        p = p.Ln
    }
    return strings.ReplaceAll(strings.Join(buf, ""), `"`, `\"`)
}
...
func cfgdot(b *GraphBuilder, bb *BasicBlock, fn string) {
    ...
    for q.Enqueue(bb); !q.Empty(); {
        ...
        for _, ln := range p.Link {
            ...
            if !r[rt] {
                buf = append(buf, fmt.Sprintf(`    BB_%d -> BB_%d`, p.Id, ln.Id))
            }
            r[rt] = true
        }
        m[p] = struct{}{}
