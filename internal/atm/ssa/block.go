...
func (self *BasicBlock) termCondition(p *hir.Ir, t *BasicBlock, f *BasicBlock) {
	...
	switch p.Op {
	case hir.OP_beq:
		cmp, lhs, rhs = IrCmpEq, p.Rx, p.Ry
	case hir.OP_bne:
		cmp, lhs, rhs = IrCmpNe, p.Rx, p.Ry
	case hir.OP_blt:
		cmp, lhs, rhs = IrCmpLt, p.Rx, p.Ry
	case hir.OP_bltu:
		cmp, lhs, rhs = IrCmpLtu, p.Rx, p.Ry
	case hir.OP_bgeu:
		cmp, lhs, rhs = IrCmpGeu, p.Rx, p.Ry
	case hir.OP_beqp:
		cmp, lhs, rhs = IrCmpEq, p.Ps, p.Pd
	case hir.OP_bnep:
		cmp, lhs, rhs = IrCmpNe, p.Ps, p.Pd
	default:
		panic("invalid branch: " + p.Disassemble(nil))
	}
