...
func (self _Stmt) negated() _Stmt {
	switch self.rel {
	case _R_eq:
		return _Stmt{self.lhs, self.rhs, _R_ne}
	case _R_ne:
		return _Stmt{self.lhs, self.rhs, _R_eq}
	case _R_lt:
		return _Stmt{self.lhs, self.rhs, _R_ge}
	case _R_ltu:
		return _Stmt{self.lhs, self.rhs, _R_geu}
	case _R_ge:
		return _Stmt{self.lhs, self.rhs, _R_lt}
	case _R_geu:
		return _Stmt{self.lhs, self.rhs, _R_ltu}
	default:
		panic("unreachable")
	}
}
...
func (self *_Range) remove(lower Int65, upper Int65) {
	for i := 0; i < len(self.rr); i += 2 {
		...
		/* not intersecting */
		if lower.Compare(u) > 0 {
			break
		}
		if upper.Compare(l) < 0 {
			continue
		}
...
func (self *_Range) intersect(lower Int65, upper Int65) {
	if lower != MinInt65 {
		self.remove(MinInt65, lower.OneLess())
	}
	if upper != MaxInt65 {
		self.remove(upper.OneMore(), MaxInt65)
	}
}
...
func (self *_Proof) verifyCorrectness() bool {
	...
	for rt {
		...
		for _, v := range sp {
			...
			/* simple ranges */
			case _R_ne:
				rr.of(v.lhs).remove(x, x)
			case _R_eq:
				rr.of(v.lhs).intersect(x, x)
			case _R_ge:
		...
		for _, v := range sp {
			...
			if t, f = v.rhs.(_RegTerm); !f {
				continue
			}
			if r, f = rr.at(Reg(t)); !f {
				continue
			}
			...
			case _R_ne:
				rr.of(v.lhs).removeRange(r)
			case _R_eq:
				rr.of(v.lhs).intersectRange(r)
		...
		case *IrBinaryExpr:
			{
				switch p.Op {
				case IrCmpEq:
					ps.assume(p.R, p.X, _RegTerm(p.Y), _R_eq)
				case IrCmpNe:
					ps.assume(p.R, p.X, _RegTerm(p.Y), _R_ne)
				case IrCmpLt:
					ps.assume(p.R, p.X, _RegTerm(p.Y), _R_lt)
				case IrCmpLtu:
					ps.assume(p.R, p.X, _RegTerm(p.Y), _R_ltu)
				case IrCmpGeu:
					ps.assume(p.R, p.X, _RegTerm(p.Y), _R_geu)
				}
			}
		}
	}
