...
func translate_OP_sint(p *hir.Builder, v Instr) {
	...
	case 8:
		...
		p.SQ(TR, TP, 0)
	default:
		panic("can only convert 1, 2, 4 or 8 bytes at a time")
	}
}
...
func translate_OP_memcpy_be(p *hir.Builder, v Instr) {
	...
	case 8:
		...
		p.SQ(UR, EP, 0)
	default:
		panic("can only swap 2, 4 or 8 bytes at a time")
	}
...
func translate_OP_unique_type(p *hir.Builder, vt *rt.GoType) {
	switch vt.Kind() {
	case reflect.Bool:
		translate_OP_unique_b(p)
	case reflect.Int:
		translate_OP_unique_int(p)
	case reflect.Int8:
		translate_OP_unique_i8(p)
	case reflect.Int16:
		translate_OP_unique_i16(p)
	case reflect.Int32:
		translate_OP_unique_i32(p)
	case reflect.Int64:
		translate_OP_unique_i64(p)
	...
	case reflect.Slice:
		break
	case reflect.String:
		translate_OP_unique_str(p)
	case reflect.Struct:
		break
	default:
		panic("unique: invalid type: " + vt.String())
	}
}
