...
func (self *CodeGen) Generate(s hir.Program, sp uintptr) *Func {
	...
	default:
		panic("pgen: program halts more than once")
	}
	...
	for v := s.Head; v != nil; v = v.Ln {
		...
		case hir.OP_gcall:
			fallthrough
		case hir.OP_icall:
			self.ctxt.require(abi.ABI.GetLayout(hir.LookupCall(v.Iv).Id).Sp)
		case hir.OP_bcopy:
			self.ctxt.require(_M_memcpyargs)
		}
	}
	...
	if i, n := 0, self.ctxt.regc(); n != 0 {
		...
		for n >= 2 {
			p.MOVDQU(XMM15, Ptr(RSP, self.ctxt.save()+int32(i)*abi.PtrSize))
			i += 2
			n -= 2
		}
		if n != 0 {
			p.MOVQ(0, Ptr(RSP, self.ctxt.save()+int32(i)*abi.PtrSize))
		}
	}
...
func (self *CodeGen) translate_OP_muli(p *x86_64.Program, v *hir.Ir) {
	...
	switch {
	case v.Iv == 0:
		self.clr(p, v.Ry) // x * 0 == 0
	case v.Iv == 1:
		self.dup(p, v.Rx, v.Ry) // x * 1 == x
	...
	case v.Iv == 2:
		if x == y {
			...
			p.LEAQ(Sib(x, x, 1, 0), y)
		}
	case v.Iv == 4:
		if x == y {
			p.SHLQ(2, y)
		} else {
			p.LEAQ(Sib(z, x, 4, 0), y)
		}
	case v.Iv == 8:
		if x == y {
			p.SHLQ(3, y)
		} else {
			p.LEAQ(Sib(z, x, 8, 0), y)
		}

	/* small multipliers, use optimized multiplication algorithm */
	case v.Iv == 3:
		p.LEAQ(Sib(x, x, 2, 0), y) // x * 3  == x + x * 2
	case v.Iv == 5:
		p.LEAQ(Sib(x, x, 4, 0), y) // x * 5  == x + x * 4
	case v.Iv == 6:
		p.LEAQ(Sib(x, x, 2, 0), y)
		p.ADDQ(y, y) // x * 6  == x * 3 * 2
	case v.Iv == 9:
		p.LEAQ(Sib(x, x, 8, 0), y) // x * 9  == x + x * 8
	case v.Iv == 10:
		p.LEAQ(Sib(x, x, 4, 0), y)
		p.ADDQ(y, y) // x * 10 == x * 5 * 2
	case v.Iv == 12:
		p.LEAQ(Sib(x, x, 2, 0), y)
		p.SHLQ(2, y) // x * 12 == x * 3 * 4
	case v.Iv == 15:
		p.LEAQ(Sib(x, x, 4, 0), y)
		p.LEAQ(Sib(y, y, 2, 0), y) // x * 15 == x * 5 * 3
	case v.Iv == 18:
		p.LEAQ(Sib(x, x, 8, 0), y)
		p.ADDQ(y, y) // x * 18 == x * 9 * 2
	case v.Iv == 20:
		p.LEAQ(Sib(x, x, 4, 0), y)
		p.SHLQ(2, y) // x * 20 == x * 5 * 4
	case v.Iv == 24:
		p.LEAQ(Sib(x, x, 2, 0), y)
		p.SHLQ(3, y) // x * 24 == x * 3 * 8
	case v.Iv == 25:
		p.LEAQ(Sib(x, x, 4, 0), y)
		p.LEAQ(Sib(y, y, 4, 0), y) // x * 25 == x * 5 * 5
	case v.Iv == 27:
		p.LEAQ(Sib(x, x, 8, 0), y)
		p.LEAQ(Sib(y, y, 2, 0), y) // x * 27 == x * 9 * 3
	case v.Iv == 36:
		p.LEAQ(Sib(x, x, 8, 0), y)
		p.SHLQ(2, y) // x * 36 == x * 9 * 4
	case v.Iv == 40:
		p.LEAQ(Sib(x, x, 4, 0), y)
		p.SHLQ(3, y) // x * 40 == x * 5 * 8
	case v.Iv == 45:
		p.LEAQ(Sib(x, x, 8, 0), y)
		p.LEAQ(Sib(y, y, 4, 0), y) // x * 45 == x * 9 * 5
	case v.Iv == 48:
		p.LEAQ(Sib(x, x, 2, 0), y)
		p.SHLQ(4, y) // x * 48 == x * 3 * 16
	case v.Iv == 72:
		p.LEAQ(Sib(x, x, 8, 0), y)
		p.SHLQ(3, y) // x * 72 == x * 9 * 8
	case v.Iv == 80:
		p.LEAQ(Sib(x, x, 4, 0), y)
		p.SHLQ(4, y) // x * 80 == x * 5 * 16
	case v.Iv == 81:
		p.LEAQ(Sib(x, x, 8, 0), y)
		p.LEAQ(Sib(y, y, 8, 0), y) // x * 81 == x * 9 * 9
