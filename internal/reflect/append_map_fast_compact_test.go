...
func TestCompactMapFastPaths(t *testing.T) {
	...
	// i16 key combos
	{
		...
		p0 := &TestStruct{
			M1: map[int16]int8{11: 1}, M2: map[int16]int16{21: 1},
			M3: map[int16]int32{31: 1}, M4: map[int16]int64{41: 1},
			M5: map[int16]EnumType{51: 1}, M6: map[int16]string{61: "1"},
			M7: map[int16]*EmptyStruct{71: {}},
		}
		doTest(t, p0, &TestStruct{})
	}
	...
	// i32 key combos
	{
		...
		p0 := &TestStruct{
			M1: map[int32]int8{11: 1}, M2: map[int32]int16{21: 1},
			M3: map[int32]int32{31: 1}, M4: map[int32]int64{41: 1},
			M5: map[int32]EnumType{51: 1}, M6: map[int32]string{61: "1"},
			M7: map[int32]*EmptyStruct{71: {}},
		}
		doTest(t, p0, &TestStruct{})
	}
	...
	// i64 key combos
	{
		...
		p0 := &TestStruct{
			M1: map[int64]int8{11: 1}, M2: map[int64]int16{21: 1},
			M3: map[int64]int32{31: 1}, M4: map[int64]int64{41: 1},
			M5: map[int64]EnumType{51: 1}, M6: map[int64]string{61: "1"},
			M7: map[int64]*EmptyStruct{71: {}},
		}
		doTest(t, p0, &TestStruct{})
	}
	...
	// enum key combos
	{
		...
		p0 := &TestStruct{
			M1: map[EnumType]int8{11: 1}, M2: map[EnumType]int16{21: 1},
			M3: map[EnumType]int32{31: 1}, M4: map[EnumType]int64{41: 1},
			M5: map[EnumType]EnumType{51: 1}, M6: map[EnumType]string{61: "1"},
			M7: map[EnumType]*EmptyStruct{71: {}},
		}
		doTest(t, p0, &TestStruct{})
	}
	...
	// string key combos
	{
		...
		p0 := &TestStruct{
			M1: map[string]int8{"11": 1}, M2: map[string]int16{"21": 1},
			M3: map[string]int32{"31": 1}, M4: map[string]int64{"41": 1},
			M5: map[string]EnumType{"51": 1}, M6: map[string]string{"61": "1"},
			M7: map[string]*EmptyStruct{"71": {}},
		}
		doTest(t, p0, &TestStruct{})
	}
	...
	// struct key combos
	{
		...
		p0 := &TestStruct{
			M1: map[*EmptyStruct]int8{{}: 1}, M2: map[*EmptyStruct]int16{{}: 1},
			M3: map[*EmptyStruct]int32{{}: 1}, M4: map[*EmptyStruct]int64{{}: 1},
			M5: map[*EmptyStruct]EnumType{{}: 1}, M6: map[*EmptyStruct]string{{}: "1"},
...
func BenchmarkCompactMapFastPath(b *testing.B) {
	type I64I64 struct {
	...
	b.Run("I64I64_64", func(b *testing.B) {
		p := mkI64I64(64)
		buf := make([]byte, 0, EncodedSizeCompact(p))
		b.ReportAllocs()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, _ = AppendCompact(buf[:0], p)
		}
	})
