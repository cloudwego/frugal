...
const gridWriteMapFileName = "gridwrite_map_gen.go"

func TestGenGridWriteMapCode(t *testing.T) {
	if *gencode {
		genGridWriteMapCode(t, gridWriteMapFileName)
		return
	}
...
func genGridWriteMapCode(t *testing.T, filename string) {
	f := &bytes.Buffer{}
	f.WriteString(gridWriteMapGenFileHeader)
	...
	for _, k := range supportTypes {
		for _, v := range supportTypes {
			fmt.Fprintf(f, "registerMapGridWriteFunc(%s, %s, %s)\n",
				t2var[k], t2var[v], gridWriteMapFuncName(k, v))
		}
	}
	fmt.Fprintln(f, "}")
	fmt.Fprintln(f, "")

	// func gridWriteMapXXX
	for _, k := range []ttype{tBYTE, tI16, tI32, tI64, tENUM, tSTRING, tOTHER} {
		for _, v := range []ttype{tBYTE, tI16, tI32, tI64, tENUM, tSTRING, tOTHER} {
			fmt.Fprintf(f, "func %s(t *tType, b *gridbuf.WriteBuffer, p unsafe.Pointer) error {\n",
				gridWriteMapFuncName(k, v))
			fmt.Fprintln(f, "n := gridWriteMapHeader(t, b, p)")
			...
			fmt.Fprintln(f, "n--")
			fmt.Fprintln(f, getGridWriteCode(k, "t.K", "kp"))
			fmt.Fprintln(f, getGridWriteCode(v, "t.V", "vp"))
			fmt.Fprintln(f, "}")
			fmt.Fprintln(f, "return checkMapN(n)")
			fmt.Fprintln(f, "}")
			fmt.Fprintln(f, "")
		}
	}
...
func gridWriteMapFuncName(k, v ttype) string {
	return fmt.Sprintf("gridWriteMap_%s_%s", ttype2FuncType(k), ttype2FuncType(v))
}

const gridWriteMapGenFileHeader = `/*
