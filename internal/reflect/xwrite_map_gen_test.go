...
const xwriteMapFileName = "xwrite_map_gen.go"

func TestGenXWriteMapCode(t *testing.T) {
	if *gencode {
		genXWriteMapCode(t, xwriteMapFileName)
		return
	}
...
func genXWriteMapCode(t *testing.T, filename string) {
	f := &bytes.Buffer{}
	f.WriteString(xwriteMapGenFileHeader)
	...
	for _, k := range supportTypes {
		for _, v := range supportTypes {
			fmt.Fprintf(f, "registerMapXWriteFunc(%s, %s, %s)\n",
				t2var[k], t2var[v], xwriteMapFuncName(k, v))
		}
	}
	fmt.Fprintln(f, "}")
	fmt.Fprintln(f, "")
	...
	for _, k := range []ttype{tBYTE, tI16, tI32, tI64, tENUM, tSTRING, tOTHER} {
		for _, v := range []ttype{tBYTE, tI16, tI32, tI64, tENUM, tSTRING, tOTHER} {
			fmt.Fprintf(f, "func %s(t *tType, b *thrift.XWriteBuffer, p unsafe.Pointer) error {\n",
				xwriteMapFuncName(k, v))
			fmt.Fprintln(f, "n := xwriteMapHeader(t, b, p)")
			...
			fmt.Fprintln(f, "n--")
			fmt.Fprintln(f, getXWriteCode(k, "t.K", "kp"))
			fmt.Fprintln(f, getXWriteCode(v, "t.V", "vp"))
			fmt.Fprintln(f, "}")
...
func xwriteMapFuncName(k, v ttype) string {
	return fmt.Sprintf("xwriteMap_%s_%s", ttype2FuncType(k), ttype2FuncType(v))
}

const xwriteMapGenFileHeader = `/*
